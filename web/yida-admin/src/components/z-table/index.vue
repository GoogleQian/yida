<template>
  <div class="z-table">
    <div class="funcs">
      <!-- 功能按钮 -->
      <el-button v-for="item in funcs" :key="item" @click="func(item)" :type="item === '新增'?'success': (item === '删除'?'danger': 'primary')">{{item}}</el-button>
    </div>
    <el-table ref="multipleTable" :data="tableData" tooltip-effect="dark" style="width: 100%" @selection-change="handleSelectionChange" border>
      ...
      <el-table-column label="操作" fixed="right" v-if="lineFuncs.length > 0">
        <template slot-scope="scope">
          ...
          <el-button size="mini" v-for="item in lineFuncs" v-if="item !== '广告审核' && item !== '删除'" :key="item" type="text" @click="lineFunc(item, scope.row)">{{item}}</el-button>
          <el-button size="mini" v-for="item in lineFuncs" v-if="item === '广告审核'" :key="item" type="text" @click="lineFunc(item, scope.row)">{{scope.row.state === 1 ? '下架': '上架'}}</el-button>
          <el-button size="mini" v-for="item in lineFuncs" v-if="item === '删除'" :key="item" type="danger" @click="lineFunc(item, scope.row)">{{item}}</el-button>
        </template>
      </el-table-column>
    </el-table>
